...
function isProxyEnabled(env = process.env) {
    return ['1', 'true', 'yes', 'on'].includes(String(env.SCHWAB_PROXY_ENABLED || '').trim().toLowerCase());
}
...
function resolveProxyUrl(env = process.env) {
    ...
    if (!env.SCHWAB_PROXY_URL) {
        throw new Error('SCHWAB_PROXY_ENABLED is true but SCHWAB_PROXY_URL is not set.');
    }

    return env.SCHWAB_PROXY_URL;
}
